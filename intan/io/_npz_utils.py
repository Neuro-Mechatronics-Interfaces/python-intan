...
# def load_npz_file(file_path: str, verbose: bool = False) -> dict:
#     """
#     Load an NPZ and reconstructs a convenient dict.
#
#     - Restores arrays exactly as saved (e.g., amplifier_data, t_amplifier, etc.).
#     - Converts object arrays (e.g., lists/dicts saved via np.savez) back to native Python
#       where reasonable (frequency_parameters, amplifier_channels, etc.).
#     - Infers/normalizes metadata: fs (Hz), _fs_Hz, n_samples, export_basename/basepath,
#       and channel_names (from amplifier_channels if needed).
#
#     Returns
#     -------
#     dict
#         Dictionary compatible with RHD result structure.
#     """
#     #if not os.path.exists(file_path):
#     #    raise FileNotFoundError(file_path)
#     if os.path.isdir(file_path):
#         file_path = list_npz_files(file_path)[0] # default to first
#
#     # allow_pickle is required if you saved dicts/lists as object arrays
#     z = np.load(file_path, allow_pickle=True)
#
...
#                     return x
#             else:
#                 # Try list() to recover sequences of dicts, etc.
#                 try:
#                     return x.tolist()
#                 except Exception:
#                     return x
#         return x
#
#     result = {k: _to_native(z[k]) for k in z.files}
#
#     # Basic file metadata
#     result['export_basename'] = os.path.splitext(os.path.basename(file_path))[0]
#     result['export_basepath'] = os.path.dirname(os.path.abspath(file_path))
#     # Keep original naming semantics if they existed
#     result.setdefault('file_name', os.path.basename(file_path))
#     result.setdefault('file_path', os.path.abspath(file_path))
#
#     # Infer sampling rate (Hz) from several possible sources
#     fs = None
...
#                 pass
#
#     # frequency_parameters dict (as saved by your RHD loader)
#     if fs is None and isinstance(result.get('frequency_parameters'), dict):
#         try:
#             fs = float(result['frequency_parameters'].get('amplifier_sample_rate'))
#         except Exception:
#             pass
#     # derive from time vector if available
#     if fs is None and isinstance(result.get('t_amplifier'), np.ndarray) and result['t_amplifier'].size > 1:
#         dt = float(np.median(np.diff(result['t_amplifier'])))
#         if dt > 0:
#             fs = 1.0 / dt
...
#     result['n_samples'] = n_samp
#
#     # Channel names convenience: prefer existing, else derive from amplifier_channels
#     if 'channel_names' not in result or not result['channel_names']:
#         ch_names = []
#         amps = result.get('amplifier_channels')
#         if isinstance(amps, (list, tuple)):
#             for ch in amps:
...
#             result['channel_names'] = ch_names
#         else:
#             # fallback generic names if we know channel count
#             if isinstance(result.get('amplifier_data'), np.ndarray):
#                 C = result['amplifier_data'].shape[0]
#                 result['channel_names'] = [f"CH{i}" for i in range(C)]
#             else:
...
#         print(f"[load_npz_file] Loaded: {file_path}")
#         print(f"  fs: {result.get('_fs_Hz')}, n_samples: {result.get('n_samples')}, "
#               f"channels: {result.get('amplifier_data').shape[0] if isinstance(result.get('amplifier_data'), np.ndarray) else 'NA'}")
#
#     return result