...
# ---------------------------------------------------------------------------
# Utilities moved to the package `intan.ui` to centralize shared UI helpers.
# The example prefers `intan.ui._helpers` and falls back to minimal
# defaults when the package or Qt isn't available (keeps the script runnable).
# ---------------------------------------------------------------------------
try:
    # Prefer package import when available
    from intan.ui._helpers import (
        which_python,
    ...
    # Last-resort minimal fallbacks to avoid hard import errors when Qt isn't
    # available or `intan.ui` isn't importable (e.g., minimal CI environment).
    ...
    def _icon(svg: str, size: int = 20):
        return QtGui.QIcon() if QtGui is not None else None
